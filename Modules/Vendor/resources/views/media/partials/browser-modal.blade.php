...
<script>
    (() => {
        ...
        async function loadBrowser(path = state.currentPath, search = state.search) {
            ...
            const response = await fetch(`{{ route('vendor.media.browser') }}?${query.toString()}`, {
        ...
        document.getElementById('media-add-folder-btn')?.addEventListener('click', async () => {
            ...
            const response = await fetch(`{{ route('vendor.media.folder') }}`, {
                method: 'POST',
                headers: {
        ...
        uploadInput?.addEventListener('change', async function () {
            ...
            const response = await fetch(`{{ route('vendor.media.upload') }}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': csrfToken(),
                    'X-Requested-With': 'XMLHttpRequest',
