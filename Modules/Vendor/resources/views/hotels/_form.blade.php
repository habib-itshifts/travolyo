@php
    $isEdit = isset($hotel);
    $action = $isEdit ? route('vendor.hotels.update', $hotel->id) : route('vendor.hotels.store');
...
<form method="POST" action="{{ $action }}" id="hotelForm" enctype="multipart/form-data">
    ...
    <div class="row g-4">
                ...
                <div class="tab-pane fade show active" id="tab-basic" role="tabpanel">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="name" id="hotelName" value="{{ old('name', $hotel->name ?? '') }}"
                        ...
                        <div class="col-md-8">
                            ...
                            <input type="text" name="slug" id="hotelSlug" value="{{ old('slug', $hotel->slug ?? '') }}"
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="short_description" value="{{ old('short_description', $hotel->short_description ?? '') }}"
                                   class="form-control @error('short_description') is-invalid @enderror"
                                   placeholder="One-line summary shown in search results" maxlength="500">
                ...
                <div class="tab-pane fade" id="tab-location" role="tabpanel">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="latitude" step="any" value="{{ old('latitude', $hotel->latitude ?? '') }}"
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="longitude" step="any" value="{{ old('longitude', $hotel->longitude ?? '') }}"
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.01" min="0" name="base_price"
                                   value="{{ old('base_price', $hotel->base_price ?? '') }}"
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.01" min="0" name="sale_price"
                                   value="{{ old('sale_price', $hotel->sale_price ?? '') }}"
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" min="0" name="min_day_before_booking"
                                   value="{{ old('min_day_before_booking', $hotel->min_day_before_booking ?? '') }}"
                                   class="form-control @error('min_day_before_booking') is-invalid @enderror"
                                   placeholder="e.g. 3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" min="0" name="min_day_stays"
                                   value="{{ old('min_day_stays', $hotel->min_day_stays ?? '') }}"
                                   class="form-control @error('min_day_stays') is-invalid @enderror"
                                   placeholder="e.g. 2">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-12">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" name="extra_prices[{{ $index }}][per_person]" value="1"
                                                           id="extra_price_person_{{ $index }}" {{ !empty($item['per_person']) ? 'checked' : '' }}>
                ...
                <div class="tab-pane fade" id="tab-policies" role="tabpanel">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="related_hotel_ids" value="{{ old('related_hotel_ids', $hotel->related_hotel_ids ?? '') }}"
                                   class="form-control @error('related_hotel_ids') is-invalid @enderror"
        ...
        {{-- Right column — Submission card only --}}
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm rounded-3 mb-3">
                <div class="card-header bg-white border-bottom fw-semibold py-3" style="font-size:13px;">Submission</div>
                <div class="card-body">
                    <div class="alert alert-info rounded-3 mb-0" style="font-size:13px;">
                        <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" class="me-1" style="vertical-align:text-bottom;">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20A10 10 0 0012 2z"/>
                        </svg>
                        Your hotel will be saved as <strong>Draft</strong> and reviewed by an admin before going live.
                    </div>
                </div>
                <div class="card-footer bg-white border-top d-flex gap-2">
                    <button type="submit" class="btn btn-sm flex-fill text-white fw-semibold"
                        ...
                        {{ $isEdit ? 'Save Changes' : 'Submit Hotel' }}
                    </button>
                    <a href="{{ route('vendor.hotels.index') }}" class="btn btn-sm btn-outline-secondary">Cancel</a>
                </div>
            </div>
...
<script>
    ...
    (() => {
        ...
        document.addEventListener('click', (event) => {
            const trigger = event.target.closest('[data-open-media-browser][data-media-target]');
            if (!trigger) return;
            activeMediaTarget = trigger.dataset.mediaTarget || null;
        });
        ...
        document.getElementById('media-use-selected-btn')?.addEventListener('click', () => {
            ...
            if (activeMediaTarget === 'gallery') {
                const hidden  = document.getElementById('gallery-image-ids');
                const preview = document.getElementById('gallery-images-preview');
                if (hidden && preview) {
                    hidden.value      = files.map((f) => f.id).join(',');
                    preview.innerHTML = files.map((f) => (
                        `<img src="${f.url}" alt="Gallery Image" class="rounded border" style="width:110px;height:90px;object-fit:cover;">`
                    )).join('');
                }
            }
        });
