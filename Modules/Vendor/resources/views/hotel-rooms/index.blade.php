<x-vendor::layouts.master title="My Hotel Rooms">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h5 class="fw-bold mb-0 text-dark">My Hotel Rooms</h5>
            <p class="text-muted mb-0" style="font-size:13px;">Add, edit, and manage rooms for your hotels.</p>
        </div>
        <a href="{{ route('vendor.hotel-rooms.create', ['hotel_id' => request('hotel_id')]) }}"
           class="btn btn-sm text-white d-inline-flex align-items-center gap-2"
    ...
    <div class="card border-0 shadow-sm rounded-3 mb-4">
        <div class="card-body py-3">
            <form method="GET" action="{{ route('vendor.hotel-rooms.index') }}">
                <div class="row g-2 align-items-end">
                    <div class="col-md-4">
                        <input type="text" name="search" value="{{ request('search') }}"
                               class="form-control form-control-sm" placeholder="Search by room name...">
                    </div>
                    <div class="col-md-4">
                        <select name="hotel_id" class="form-select form-select-sm">
                            <option value="">All My Hotels</option>
                            @foreach ($hotels as $hotel)
                                <option value="{{ $hotel->id }}" {{ (string) request('hotel_id') === (string) $hotel->id ? 'selected' : '' }}>
                                    {{ $hotel->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 d-flex gap-2">
                        ...
                        <a href="{{ route('vendor.hotel-rooms.index') }}" class="btn btn-sm btn-outline-secondary px-3">Reset</a>
                    </div>
                </div>
            </form>
        </div>
                            ...
                            <tr>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-2">
                                        <a href="{{ route('vendor.hotel-rooms.edit', $room->id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted py-5">
                                    ...
                                    <p class="mb-1">No rooms found.</p>
                                    <a href="{{ route('vendor.hotel-rooms.create', ['hotel_id' => request('hotel_id')]) }}"
                                       class="btn btn-sm mt-2 text-white" style="background:var(--clr-primary);">Add New Room</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
