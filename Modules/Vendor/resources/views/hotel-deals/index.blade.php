<x-vendor::layouts.master :title="'Deals — ' . $hotel->name">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <nav aria-label="breadcrumb" style="font-size:12px;">
                <ol class="breadcrumb mb-1">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('vendor.hotels.show', $hotel->id) }}" class="text-decoration-none">{{ Str::limit($hotel->name, 25) }}</a></li>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('vendor.hotels.deals.create', $hotel->id) }}" class="btn btn-sm text-white d-inline-flex align-items-center gap-2"
            ...
            <a href="{{ route('vendor.hotels.show', $hotel->id) }}" class="btn btn-sm btn-outline-secondary">Back to Hotel</a>
        </div>
    </div>

    <div class="card border-0 shadow-sm rounded-3 mb-4">
        <div class="card-body py-3">
            <form method="GET" action="{{ route('vendor.hotels.deals.index', $hotel->id) }}">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        ...
                        <a href="{{ route('vendor.hotels.deals.index', $hotel->id) }}" class="btn btn-sm btn-outline-secondary px-3">Reset</a>
                    </div>
                </div>
            </form>
        </div>
                            ...
                            <tr>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-1">
                                        <a href="{{ route('vendor.hotels.deals.edit', [$hotel->id, $deal->id]) }}"
                                           class="btn btn-sm btn-outline-primary" title="Edit">
                                            <svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <form method="POST" action="{{ route('vendor.hotels.deals.destroy', [$hotel->id, $deal->id]) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted py-5">
                                    ...
                                    <div class="mt-2">
                                        <a href="{{ route('vendor.hotels.deals.create', $hotel->id) }}" class="btn btn-sm"
