<x-vendor::layouts.master>
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">All flight and hotel bookings for your listings.</p>
        </div>
    ...
    <div class="d-flex gap-2 mb-4 flex-wrap">
        @foreach (['all' => 'All Bookings', 'flight' => 'Flights', 'hotel' => 'Hotels'] as $key => $label)
            @php
                ...
                $isActive = $type === $key;
            @endphp
            <a href="{{ route('vendor.bookings.index', array_merge(request()->except('type','page'), ['type' => $key])) }}"
    ...
    <form method="GET" action="{{ route('vendor.bookings.index') }}" class="d-flex gap-2 flex-wrap mb-4">
        ...
        @if($search || $status)
            <a href="{{ route('vendor.bookings.index', ['type' => $type]) }}"
                    ...
                    <table class="table table-hover align-middle mb-0" style="font-size:13px;">
                        <thead>
                            <tr class="text-uppercase text-muted border-bottom" style="font-size:10px;letter-spacing:.05em;">
                                ...
                                <th class="fw-semibold py-3">Travel Date</th>
                                <th class="fw-semibold py-3">Amount</th>
                                <th class="fw-semibold py-3">Your Earnings</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-semibold" style="color:#15803d;">
                                            {{ strtoupper($booking->currency ?? 'USD') }}
                                            {{ number_format($booking->vendor_amount ?? 0, 2) }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge rounded-pill fw-semibold"
