<x-admin::layouts.master title="Spaces">

    {{-- Page Header --}}
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h5 class="fw-bold mb-0 text-dark">Spaces</h5>
            <p class="text-muted mb-0" style="font-size:13px;">Manage all space listings</p>
        </div>
        <a href="{{ route('admin.spaces.create') }}" class="btn btn-sm text-white d-inline-flex align-items-center gap-2"
    ...
    <div class="card border-0 shadow-sm rounded-3 mb-4">
        <div class="card-body py-3">
            <form method="GET" action="{{ route('admin.spaces.index') }}">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        ...
                        <a href="{{ route('admin.spaces.index') }}" class="btn btn-sm btn-outline-secondary px-3">Reset</a>
                    </div>
                </div>
            </form>
        </div>
                ...
                <table class="table table-hover mb-0 align-middle" style="font-size:13px;">
                    ...
                    <tbody>
                        @forelse ($spaces as $space)
                            <tr class="{{ $space->trashed() ? 'opacity-50' : '' }}">
                                <td class="ps-4 text-muted">{{ $space->id }}</td>
                                <td>
                                    <a href="{{ route('admin.spaces.show', $space->id) }}" class="d-flex align-items-center gap-2 text-decoration-none">
                                ...
                                <td><span class="fw-semibold">{{ $space->author?->name ?? '—' }}</span></td>
                                <td><span class="badge bg-light text-dark border text-capitalize">{{ $space->type }}</span></td>
                                <td>
                                    <p class="mb-0">{{ $space->city }}, {{ $space->country }}</p>
                                    <p class="mb-0 text-muted" style="font-size:11px;">{{ Str::limit($space->address, 35) }}</p>
                                </td>
                                <td>
                                    @if ($space->sale_price && $space->sale_price < $space->price_per_night)
                                        <span class="text-decoration-line-through text-muted">{{ $space->currency }} {{ number_format($space->price_per_night, 2) }}</span>
                                        <br>
                                ...
                                <td>
                                    <span class="text-muted" style="font-size:12px;">{{ $space->max_guests }} guests, {{ $space->bedrooms }} bed, {{ $space->bathrooms }} bath</span>
                                </td>
                                <td>
                                    @php
                                ...
                                <td class="text-end pe-4">
                                    @if ($space->trashed())
                                        <form method="POST" action="{{ route('admin.spaces.restore', $space->id) }}" class="d-inline">
                                            @csrf
                                    ...
                                    @else
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end" style="font-size:13px;">
                                                <li><a class="dropdown-item" href="{{ route('admin.spaces.availability', $space->id) }}">Manage Availability</a></li>
                                                <li><hr class="dropdown-divider"></li>
                                                <li><a class="dropdown-item" href="{{ route('admin.spaces.edit', $space->id) }}">Edit Space</a></li>
                                                <li>
                                                    <form method="POST" action="{{ route('admin.spaces.destroy', $space->id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted py-5">
                                    <p class="mb-0">No spaces found.</p>
                                    <a href="{{ route('admin.spaces.create') }}" class="btn btn-sm mt-2"
                                       style="background:var(--clr-primary);color:#fff;">Add First Space</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
