@php
    ...
    $galleryMedia = $galleryIds->isNotEmpty()
        ...
        : collect();
    $galleryImages = $galleryIds->map(fn ($id) => optional($galleryMedia->get($id))->url)->filter()->values()->all();
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="checkbox-chip">
                                    <input type="checkbox" name="instant_confirmation" value="1" {{ old('instant_confirmation', $activity->instant_confirmation ?? true) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-4">
                                <label class="checkbox-chip">
                                    <input type="checkbox" name="email_flyer_enabled" value="1" {{ old('email_flyer_enabled', $activity->email_flyer_enabled ?? true) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex gap-2 mb-2">
                                <button type="button" class="btn btn-info btn-sm text-white" data-open-media-browser data-media-target="gallery" data-media-multiple="true">
                                    Select images
                                </button>
                            </div>
                    ...
                    <div class="d-grid gap-2 mb-4">
                        @foreach (['publish' => 'Publish', 'draft' => 'Draft'] as $value => $label)
                            <label class="status-radio">
                                <input type="radio" name="status" value="{{ $value }}" {{ old('status', $activity->status ?? 'publish') === $value ? 'checked' : '' }}>
                                <span>{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="author_id" class="form-select @error('author_id') is-invalid @enderror">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ (string) old('author_id', $activity->author_id ?? auth()->id()) === (string) $user->id ? 'selected' : '' }}>
...
<template id="extra-information-template">
    <div class="border rounded-3 p-3 extra-info-item" data-extra-info-item>
        <div class="row g-3 align-items-end">
            <div class="col-md-11">
                <label class="form-label fw-semibold" style="font-size:13px;">Item</label>
                <input type="text" name="extra_information[__INDEX__]" class="form-control" placeholder="e.g. Private transport, entry tickets, professional guide">
            </div>
            <div class="col-md-1">
                <button type="button" class="btn btn-sm btn-outline-danger w-100" data-remove-extra-info>x</button>
            </div>
...
<script>
    (() => {
        ...
        document.addEventListener('click', (event) => {
            const removeButton = event.target.closest('[data-remove-extra-info]');
            if (!removeButton) {
                return;
            }

            const item = removeButton.closest('[data-extra-info-item]');
