...
class HyperguestHotelProvider implements HotelProviderInterface
{
    ...
    {
        //step 1: load all Hotels- from static api we will get all hotels and then we will filter with required destination
        ...
        // Step 2: Filter + get hotel IDs
        $hotelIds = $hotelsList
            ->filter(function ($h) use ($destination) {
        ...
        //setp 3: batch Hotel IDs (important for API Limits)
        $chunks = $hotelIds->chunk(10);


        $hotelWithRooms = collect();

        foreach($chunks as $chunk)
        {
           
            $idsString = $chunk->implode(',');
            ...
            ->get('https://search-api.hyperguest.io/2.0/', [
                ...
                'guests' => $dto->guests(),
                'hotelIds' => $idsString,
                'customerNationality' => $dto->nationality,
        ...
        return $hotelWithRooms->values()->all();
